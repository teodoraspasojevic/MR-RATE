...
# ==================== CT-CLIP worst-of-K candidate filter ====================
# Inverted best-of-K: generate FILTER_K candidates per prompt and keep the one
# whose CT-CLIP image embedding matches the prompt WORST, not best. Best-of-K
# (see inference_ft_vae11k_v2.py) raised CLIPScore but tanked FVD_CTNet on the
# real leaderboard; this tests whether that relationship is real and inverted
# by deliberately selecting the opposite end of the same score.
# Exactly one volume is still written per prompt.
FILTER_K = int(os.environ.get("CTFLOW_FILTER_K", "3"))
...
def main():
    ...
    # base seed for the worst-of-K candidate draws; only used when FILTER_K > 1,
    ...
    # ---- optional CT-CLIP worst-of-K scorer (FILTER_K=1 -> disabled entirely) ----
    scorer = None
    if FILTER_K > 1:
        try:
            scorer = CTCLIPImageScorer(TEXT_MODEL_PATH, TOKENIZER_NAME, device)
            if rank == 0:
                print(f"[filter] CT-CLIP worst-of-{FILTER_K} enabled", flush=True)
        except Exception as exc:
    ...
    for start in tqdm(range(0, len(my_idx), BATCH_SIZE), desc=f"Rank {rank} generating", disable=(rank != 0)):
        ...
        # ---- worst-of-K: score this candidate, then draw K-1 more and keep the WORST ----
        # Inverted from best-of-K on purpose (see module docstring above). Quality
        # gate still wins over score either way: a short/failed candidate never
        # beats a quality-passing one, we just prefer the lowest-scoring passer
        # instead of the highest. `(ok, -s)` maximized == prefer ok=True, then
        # prefer the smallest s.
            ...
            try:
                ...
                if rank == 0:
                    print(f"[filter] batch CLIPScore mean {100*float(np.mean(best_score)):.2f} "
                          f"(worst of {FILTER_K})", flush=True)
                vol_list = [a.unsqueeze(0) for a in best_arr]   # already trimmed & converted to HU
        ...
        for j, n in enumerate(name_batch):
            arr = vol_list[j][0]  # take first channel, (T,H,W) in [-1,1] unless _already_scaled
