...
def main():
    ...
    if rank == 0:
        print(f"[select] {NOISE_K} candidates/prompt; length>={NOISE_MIN_FRAMES} gate -> "
              ...
              f"< {MOTION_FLOOR_RATIO}x median -> keep jerk closest to group median (ties: longer)", flush=True)
    ...
    for start in tqdm(range(0, len(my_idx), BATCH_SIZE), desc=f"Rank {rank} generating", disable=(rank != 0)):
        ...
        for k in range(NOISE_K):
            # k=0 is seeded too (upstream skipped it, leaving the first candidate
            # at whatever OS-entropy state the process started with -- that made
            # every run irreproducible and was measured to swing the CTNet
            # Frechet proxy by ~22% on identical config). Seeding it makes
            # CTFLOW_SEED control the whole run.
        ...
        for j, n in enumerate(name_batch):
            ...
            # Closest-to-median jerk, not lowest: v10 (argmin jerk) improved
            # FID/CLIP noticeably (esp. XZ/YZ, exactly what this was meant to
            # fix) but made FVD *worse* -- picking the single smoothest-of-K
            # outlier likely favors atypically simple/low-structure content
            # over the natural level of z-direction variation real anatomy
            # has, which a volumetric metric like FVD is more sensitive to
            # than per-plane FID. Targeting the group's typical jerk instead
            # of its minimum should keep the XZ/YZ fix without over-smoothing.
            median_jerk = float(np.median([jerks[i] for i in keep]))
            best_i = min(keep, key=lambda i: (abs(jerks[i] - median_jerk), -lengths[i]))
