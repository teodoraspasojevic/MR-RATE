...
BLOCK_SIZE = 16
GUIDANCE_SCALE = float(os.environ.get("CTFLOW_GUIDANCE_SCALE", "1.0"))  # 1.0 = no CFG
    ...
    def generate(self, prompt_emb: torch.Tensor, max_blocks: int = 16):
        ...
        with torch.no_grad(), autocast(dtype=self.dtype):
            for _ in range(max_blocks):
                ...
                # Classifier-free guidance, matching train_ft.py's log_validation
                # new_forward: both text and image conditioning are dropped
                # (independently, each w.p. p_drop_conditionning=0.1) during
                # training, so the denoiser has seen null_text/null_image and a
                # null+null combo -- v_uncond here should be in-distribution.
                # guidance_scale=1.0 collapses algebraically to v_cond alone
                # (v_uncond cancels out), so skip the extra forward pass then.
                ...
                if gscale == 1.0:
                    ...
                    null_text = torch.zeros_like(prompt_emb)
                    null_image = torch.zeros_like(cond_lat)

                    def rhs(t, y):
                        v_cond = self.denoiser(
                            y, t, encoder_hidden_states=prompt_emb, cond_image=cond_lat
                        ).sample
                        v_uncond = self.denoiser(
                            y, t, encoder_hidden_states=null_text, cond_image=null_image
                        ).sample
                        return v_uncond + gscale * (v_cond - v_uncond)
...
def main():
    ...
    if rank == 0:
        print(f"[guidance] guidance_scale={GUIDANCE_SCALE}", flush=True)
    gen = LatentAutoregressiveGenerator(denoiser, vae, vae_scaling, cfg, guidance_scale=GUIDANCE_SCALE)
