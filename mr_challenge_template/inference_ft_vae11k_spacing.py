...
# https://huggingface.co/datasets/dmolino/CT-RATE_Generated_Scans (public data card):
#   spacing (0.75, 0.75, 3.0) mm, size (512, 512, 128), physical FOV 384x384x384 mm,
#   HU [-1000, 1000].
# Our raw output is (T,256,256) with T variable (~100-272). We don't carry explicit
# physical spacing -- 1.5mm isotropic is the existing assumption already used
# elsewhere in this codebase (inference_ft_vae11k_v2.py's CTCLIPImageScorer._CLIP_SPACING,
# itself sourced from metrics_scaled/evaluate_dist_ctclip.py), giving a 256*1.5=384mm
# FOV that matches the target's 384mm FOV exactly -- same physical box, different grid.
_NATIVE_SPACING = (1.5, 1.5, 1.5)      # assumed (x,y,z) mm of our raw output
_TARGET_SPACING = (0.75, 0.75, 3.0)    # ArcoLab spec (x,y,z) mm
_TARGET_SIZE = (512, 512, 128)         # ArcoLab spec (x,y,z) voxels
_TARGET_DEFAULT_HU = -1000.0           # pad value when native FOV < target FOV

# Zero-padding would need 384/1.5=256 native frames, but that's right up
# against the generator's T=272 cap and made most samples retry repeatedly.
# 245 frames leaves a tolerated gap of (256-245)*1.5=16.5mm (~5-6 slices of
# -1000 padding split across both ends, out of 128) -- small relative to the
# volume, and reachable without hammering retries on nearly every sample.
SPACING_MIN_FRAMES = 245
# Bounded, not infinite: a prompt that never reaches SPACING_MIN_FRAMES would
# otherwise retry forever and eat the whole time budget alone. Keep the
# longest candidate seen across attempts if the cap is hit.
SPACING_MAX_RETRIES = 8


def resample_to_arcolab_spec(
    volume: torch.Tensor,
    ...
    """(T,H,W) HU tensor -> resampled sitk.Image at the ArcoLab spec, centered,
    linear interpolation, clamped to [-1000, 1000], float32."""
    if torch.is_tensor(volume):
    ...
    old_center_index = (old_size - 1.0) / 2.0
    old_center = np.array(image.TransformContinuousIndexToPhysicalPoint(old_center_index.tolist()))

    direction = np.array(image.GetDirection(), dtype=np.float64).reshape(3, 3)
    new_center_offset = direction @ (((target_size_arr - 1.0) * target_spacing_arr) / 2.0)
    target_origin = old_center - new_center_offset
...
def save_as_niigz_arcolab(volume: torch.Tensor, path: str):
    """Resample to the ArcoLab spec, then write compressed float32 NIfTI."""
    output = resample_to_arcolab_spec(volume)
...
def main():
    ...
    for start in tqdm(range(0, len(my_idx), BATCH_SIZE), desc=f"Rank {rank} generating", disable=(rank != 0)):
        ...
        # Retry any sample whose blank-trimmed volume is too short to fill the
        # ArcoLab target's z-depth within the tolerated padding (SPACING_MIN_FRAMES=245).
        # This subsumes the plain quality gate (100 frames) since 245 > 100.
        # Track the longest candidate seen so far so a prompt that never
        # reaches 245 within SPACING_MAX_RETRIES still gets the best attempt,
        # not just whichever one happened to run last.
        for j, n in enumerate(name_batch):
            best = vol_list[j]
            best_trimmed = trim_blank_tail(best[0])
            retries = 0
            while best_trimmed.shape[0] < SPACING_MIN_FRAMES and retries < SPACING_MAX_RETRIES:
                retries += 1
                if rank == 0:
                    print(
                        f"[spacing] {n}: only {best_trimmed.shape[0]} slices "
                        f"(<{SPACING_MIN_FRAMES} needed to avoid padding), "
                        f"regenerating (attempt {retries}/{SPACING_MAX_RETRIES})",
                        flush=True,
                    )
                redo = gen.generate_and_decode(embeds[j : j + 1], max_blocks=17)
                cand_trimmed = trim_blank_tail(redo[0][0])
                if cand_trimmed.shape[0] > best_trimmed.shape[0]:
                    best, best_trimmed = redo[0], cand_trimmed
            vol_list[j] = best
            if retries and best_trimmed.shape[0] < SPACING_MIN_FRAMES and rank == 0:
                print(
                    f"[spacing] {n}: best of {retries+1} attempts still only "
                    f"{best_trimmed.shape[0]} slices, keeping it (will be padded)",
                    flush=True,
                )
        del embeds
        ...
        for j, n in enumerate(name_batch):
            ...
            pool.submit(save_as_niigz_arcolab, arr, path)
        del vol_list
