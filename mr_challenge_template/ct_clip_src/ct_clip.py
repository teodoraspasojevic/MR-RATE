...
class CTCLIP(nn.Module):
    ...
    #    #pt = torch.load(str(path), weights_only=False, map_location="cpu")
    ...
    def load(self, path):
        ...
        # 尝试兼容旧 pt 文件
        try:
            pt = torch.load(str(path), map_location="cpu", weights_only=False)
        except TypeError:
            # 兼容老版本 torch.load 不支持 weights_only 参数
